...
export class LiveStreamService {
  ...
  public async getPlaylist(
    ...
    try {
      ...
      const fetchPlaylist = async (url: string, isSubpath: boolean = false) => {
        ...
        if (!isSubpath && [301, 302, 403].includes(res.status)) {
          ...
          if (newMasterUrl) {
            ...
            const newBaseUrl = finalUrl.substring(
              0,
      ...
      if (
        subpath &&
        record.subpathRedirects &&
        record.subpathRedirects.has(subpath)
      ) {
      ...
      if (play === "1" && subpath) {
        logger.info(
          `[LiveStreamService] Fetching media playlist for subpath: ${subpath}`,
        );
        ...
        if (res.status < 200 || res.status >= 300 || !res.data) {
          logger.warn(
            `[LiveStreamService] Subpath fetch failed (status ${res.status}). Refreshing master URL...`,
          );
          ...
          if (
            ...
            return {
              error: `Upstream Error ${refreshedRes.status}`,
          ...
          for (const line of lines) {
            if (line.trim().endsWith(".m3u8")) {
              if (!newSubpath) newSubpath = line.trim();
        ...
        const finalVariantUrl =
          res.request?.res?.responseUrl || res.config?.url;
        if (finalVariantUrl) {
          record.variantBaseUrl = finalVariantUrl.substring(
            0,
            finalVariantUrl.lastIndexOf("/") + 1,
          );
        }
        ...
        logger.info(
          `[LiveStreamService] Generated media playlist (first 5 lines):\n${modifiedLines.slice(0, 5).join("\n")}`,
        );

        if (output.includes("#EXT-X-STREAM-INF")) {
          logger.warn(
            "[LiveStreamService] WARNING: Returned content looks like a Master Playlist but play=1 was requested!",
          );
        }
        ...
        logger.info(
          `[LiveStreamService] Returning master playlist for ${cmd} (play=${play})`,
        );
        const masterUrl = await serverManager
          .getProvider()
          .getChannelLink(cmd)
          .then((res) => res.js.cmd);
